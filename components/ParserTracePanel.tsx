              ...
              return (
                ...
                >
                  <div className="mb-0.5 flex items-center justify-between gap-2">
                    <span className="font-mono text-[12px] text-zinc-200">{step.rule}</span>
                    <span className="font-mono text-[11px] text-zinc-500">L{step.line}</span>
                  </div>
                  <div className="font-mono text-[12px] text-zinc-400">{step.description}</div>
                </button>
              );
            })
          )}
      ...
      <div className="min-h-0 overflow-auto rounded-xl border border-white/10 bg-black/20 p-2">
        ...
        {trace.length === 0 ? (
          ...
          <div className="space-y-2 font-mono text-[12px]">
            <div className="rounded border border-white/10 px-2 py-1.5 text-zinc-200">
              {trace[activeIndex]?.rule}
            </div>
            <div className="rounded border border-white/10 px-2 py-1.5 text-zinc-400">
              {trace[activeIndex]?.description}
            </div>
            <div className="rounded border border-white/10 px-2 py-1.5 text-zinc-500">
